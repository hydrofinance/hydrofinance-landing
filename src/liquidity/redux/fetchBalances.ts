...
export const fetchBalances = createAsync<
  ...
  TokensMap,
  Error
>("FETCH_BALANCES", async ({ networkId, address, web3 }, _1, store) => {
  const tokens = store().liquidity.tokens;
...
export function useFetchBalances() {
  ...
  const { tokens, fetchBalancesPending, fetchBalancesDone } = useSelector(
    (state: Store) => ({
      tokens: state.liquidity.tokens,
      fetchBalancesPending: state.liquidity.fetchBalancesPending,
      fetchBalancesDone: state.liquidity.fetchBalancesDone,
    }),
    shallowEqual
  );
  ...
  const boundAction = useCallback(
    () => {
      return dispatch(fetchBalances({ networkId, address, web3 }));
    },
    [dispatch, networkId, address, web3]
  );

  const tokenBalance = (tokenAddress: string | null) => {
    return byDecimals(
      tokenAddress ? tokens[tokenAddress]?.balance || 0 : 0,
      tokenAddress ? tokens[tokenAddress]?.decimals : 18
    );
  };

  const routerAddress = getHydroRouter(networkId || 0);
  const tokenAllowance = (tokenAddress: string | null) => {
    ...
    return new BigNumber(token.allowance[routerAddress] || 0);
  };

  return {
    hydroAddress: getNetworkV2TokenAddress(networkId || 1285),
    finnAddress: getFinnAddress(networkId || 1285),
    tokens,
    tokenBalance,
    tokenAllowance,
...
export const builderHandler = (
  ...
  builder
    ...
    .case(fetchBalances.async.failed, (state) =>
      R.mergeDeepRight(state, {
